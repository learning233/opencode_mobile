...
void main() {
  ...
  group('Vision Model Selection & Properties', () {
    ...
    test('picks the first vision model when no preference is configured', () {
      ctrl.availableModels.assignAll([
        modelTextOnly,
        modelVision1,
        modelVision2,
      ]);
    ...
    test(
      'falls back to default model if configured model is not found',
      () async {
  ...
  group('describeImagesToText - Precondition Checks', () {
    test(
      'returns null immediately when image list is empty without HTTP calls',
      () async {
        ctrl.availableModels.assignAll([modelVision1]);

        var requestMade = false;
        OpenCodeClient().dio.httpClientAdapter = FakeHttpAdapter((
          options,
          body,
        ) async {
          requestMade = true;
    ...
    test(
      'returns null immediately when no vision model is available without HTTP calls',
      () async {
        ctrl.availableModels.assignAll([modelTextOnly]);
  ...
  group('describeImagesToText - Polling Fallback Flow', () {
    test(
      'happy path: creates session, sends prompt, polls completed message, deletes session',
      () async {
        ctrl.availableModels.assignAll([modelVision1]);
        ...
        OpenCodeClient().dio.httpClientAdapter = FakeHttpAdapter((
          ...
          if (options.method == 'POST' &&
              path == ApiEndpoints.sessionPromptAsync(tempSessionId)) {
            return ResponseBody.fromString(
          ...
          if (options.method == 'GET' &&
              path == ApiEndpoints.sessionMessages(tempSessionId)) {
            final messages = [
              {
                'id': 'msg_user_1',
          ...
          if (options.method == 'DELETE' &&
              path == ApiEndpoints.sessionDelete(tempSessionId)) {
            return ResponseBody.fromString(
              jsonEncode({'status': 'ok'}),
              200,
              headers: {
        ...
        final methods = requests
            .map((r) => '${r.options.method} ${r.options.path}')
            .toList();
        expect(methods, contains('POST ${ApiEndpoints.sessions}'));
        expect(
          methods,
          contains('POST ${ApiEndpoints.sessionPromptAsync(tempSessionId)}'),
        );
        expect(
          methods,
          contains('GET ${ApiEndpoints.sessionMessages(tempSessionId)}'),
        );
        expect(
          methods,
          contains('DELETE ${ApiEndpoints.sessionDelete(tempSessionId)}'),
        );

        // 验证 Prompt 请求体格式
        final promptReq = requests.firstWhere(
          (r) =>
              r.options.path == ApiEndpoints.sessionPromptAsync(tempSessionId),
        );
        final promptBody =
            jsonDecode(utf8.decode(promptReq.body)) as Map<String, dynamic>;
        expect(promptBody['agent'], 'plan');
        expect(promptBody['model'], {
          'providerID': 'openai',
          'modelID': 'gpt-4o',
        });
        ...
        expect(
          (parts[1]['url'] as String).startsWith('data:image/png;base64,'),
          isTrue,
        );
      },
    );
    ...
    test(
      'polling: returns text when 4 consecutive polls return identical non-completed text',
      () async {
        ctrl.availableModels.assignAll([modelVision1]);
  ...
  group('describeImagesToText - Error Handling & Resource Cleanup', () {
    ...
    test(
      'cleans up temporary session when prompt async fails with 500',
      () async {
        ctrl.availableModels.assignAll([modelVision1]);
        ...
        OpenCodeClient().dio.httpClientAdapter = FakeHttpAdapter((
          ...
          if (options.method == 'POST' &&
              options.path == ApiEndpoints.sessions) {
            return ResponseBody.fromString(
              jsonEncode({'id': tempSessionId}),
              200,
              headers: {
          ...
          if (options.method == 'DELETE' &&
              options.path == ApiEndpoints.sessionDelete(tempSessionId)) {
            deleteCalled = true;
            return ResponseBody.fromString(jsonEncode({'status': 'ok'}), 200);
          }
    ...
    test(
      'does not throw unhandled exception if DELETE cleanup fails',
      () async {
        ctrl.availableModels.assignAll([modelVision1]);
    ...
    test(
      'multiple images: sends all images as separate parts with correct filenames and mime',
      () async {
        ctrl.availableModels.assignAll([modelVision1]);
        ...
        final bodyMap =
            jsonDecode(utf8.decode(capturedPromptReq!.body))
    ...
    test(
      'ignores empty assistant messages and reasoning parts, extracting final content',
      () async {
        ctrl.availableModels.assignAll([modelVision1]);
                  ...
                  'parts': [
                    {
                      'id': 'p_r',
                      'type': 'reasoning',
                      'text': 'Thinking about the image...',
                    },
                    {
                      'id': 'p_t',
                      'type': 'text',
                      'text': 'The final OCR text extracted.',
                    },
                  ],
                  'info': {
                    'time': {'completed': 999},
