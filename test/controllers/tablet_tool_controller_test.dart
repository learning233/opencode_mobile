...
void main() {
  group('TabletToolController content cache', () {
    ...
    test('cacheFileContent does not replace the OpenedFile in openedFiles', () {
      ctrl.cacheFileContent('lib/a.dart', 'hello');
      final file = ctrl.openedFiles.firstWhere(
        (f) => f.path == 'lib/a.dart',
      );
      expect(file.initialContent, isNull);
    });

    test('cacheFileContent works for any path (independent of openedFiles)', () {
  ...
  group('TabletToolController cross-worktree tabs', () {
    test('same relative path from different worktrees are distinct tabs', () {
      ...
      expect(ctrl.activeFileKey, TabletToolController.fileKey('lib/a.dart', '/projB'));
    });

    test('re-opening the same worktree+path activates instead of duplicating', () {
      final ctrl = TabletToolController();
      ctrl.openFile('lib/a.dart', 'a.dart', worktree: '/projA');
      ctrl.openFile('lib/a.dart', 'a.dart', worktree: '/projB');
      ctrl.openFile('lib/a.dart', 'a.dart', worktree: '/projA');
      expect(ctrl.openedFiles.length, 2);
      expect(ctrl.activeFileKey, TabletToolController.fileKey('lib/a.dart', '/projA'));
    });
  ...
  group('TabletToolController review tab switching', () {
    test('openReviewMessageFile in same scope switches to review tab and shows panel', () {
      final ctrl = TabletToolController();
    ...
    test('openReviewMessageFile in a different scope refetches and switches', () {
      final ctrl = TabletToolController();
      ctrl.activeTabIndex.value = TabletToolController.tabCode;
    ...
    test('openReviewSessionFile in same scope switches to review tab and shows panel', () {
      final ctrl = TabletToolController();
    ...
    test('openReviewSessionFile in a different scope refetches and switches', () {
      final ctrl = TabletToolController();
      ctrl.activeTabIndex.value = TabletToolController.tabCode;
      final tickBefore = ctrl.reviewReloadTick.value;
    ...
    test('openReviewSession with switchTab=false refreshes data without switching tab', () {
      final ctrl = TabletToolController();
      ctrl.activeTabIndex.value = TabletToolController.tabCode;
      ctrl.isVisible.value = false;
      final tickBefore = ctrl.reviewReloadTick.value;
  ...
  group('TabletToolController browser tabs', () {
    test('openUrl with empty url switches to web tab without creating a tab', () {
      final ctrl = TabletToolController();
      ctrl.activeTabIndex.value = TabletToolController.tabCode;
      ctrl.isVisible.value = false;
    ...
    test('openUrl with an existing normalized URL switches instead of duplicating', () {
      final ctrl = TabletToolController();
      ctrl.openUrl('https://example.com/a');
