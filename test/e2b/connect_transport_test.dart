...
void main() {
  group(
    'ConnectTransport Protocol Tests (Replicated from rpc.test.ts / connect.test.ts)',
    () {
      ...
      test('decode multiple streamed frames in single buffer', () {
        final frame1 = ConnectTransport.encodeFrame({
          'event': 'start',
          'pid': 1,
        });
        final frame2 = ConnectTransport.encodeFrame({
          'event': 'data',
          'stdout': 'aGVsbG8=',
        });
      ...
      test(
        'decodeFrames ignores incomplete trailing bytes without crashing',
        () {
