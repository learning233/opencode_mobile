...
void main() {
  group('ConnectionConfig Tests (Replicated from connectionConfig.test.ts)', () {
    ...
    test('getSandboxEnvdUrl routes by direct domain ({port}-{sandboxId})', () {
    ...
    test('getSandboxEnvdUrl uses explicit sandboxUrl override when provided',
        () {
      const config = ConnectionConfig(
        apiKey: 'key',
    ...
    test('getEnvdHeaders (unary) uses application/json Content-Type', () {
      ...
      final headers = config.getEnvdHeaders(sandboxId: 'sbx-test', port: 49983);
    ...
    test('getEnvdHeaders (streaming) uses connect+json with ping interval', () {
      const config = ConnectionConfig(
        apiKey: 'e2b_5678',
        envdAccessToken: 'envd_token_abc',
      );
