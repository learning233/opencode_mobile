...
void main() {
  group('E2B Timeout Dynamic Fallback Tests', () {
    test('sanitizeTimeoutSeconds allows 600s to 86400s (up to 24h for Pro)', () {
    ...
    test('Sandbox.create retries with 3600s when 400 timeout error is returned', () async {
      int requestCount = 0;
    ...
    test('Sandbox.create succeeds on first try for Pro users (no fallback needed)', () async {
      int requestCount = 0;
      final mockAdapter = MockDioAdapter(
        handler: (options) {
          requestCount++;
    ...
    test('Sandbox.connect retries with 3600s when 400 timeout is returned', () async {
      int requestCount = 0;
      final mockAdapter = MockDioAdapter(
        handler: (options) {
          requestCount++;
    ...
    test('Sandbox.setTimeout retries with 3600s when 400 timeout is returned', () async {
      int requestCount = 0;
      final mockAdapter = MockDioAdapter(
        handler: (options) {
          requestCount++;
