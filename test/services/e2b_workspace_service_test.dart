...
void main() {
  ...
  group('CloudWorkspaceConfig Tests', () {
    ...
    test('serialization and deserialization roundtrip', () {
      ...
      expect(
        deserialized.activeSandboxPassword,
        original.activeSandboxPassword,
      );
      expect(
  ...
  group('E2bWorkspaceService Tests', () {
    test(
      'generateSecurePassword generates distinct string of requested length',
      () {
  ...
  group('AppSettingsStore CloudWorkspace Integration', () {
    ...
    test(
      'persists selfHostedServer settings and preserves them when connecting to cloud',
      () async {
        SharedPreferences.setMockInitialValues({});
        final prefs = await SharedPreferences.getInstance();
  ...
  group('GitRepoService Tests', () {
    ...
    test(
      'buildAuthenticatedCloneUrl retains custom port and handles other platforms',
      () {
        final service = GitRepoService.instance;
        ...
        expect(giteeUrl, 'https://gitee_token@gitee.com/user/project.git');
      },
    );
  });
  ...
  group('E2bSandboxInfo Tests', () {
    test(
      'E2bSandboxInfo fromJson parses standard E2B listed sandbox correctly',
      () {
        final json = {
  ...
  group('E2bWorkspaceService health polling (mock HTTP)', () {
    ...
    test(
      'waitForHealthy reports timeout reason after exhausting retries',
      () async {
        service.healthDioForTest = Dio()
          ..httpClientAdapter = FakeHttpAdapter((options, body) async {
  ...
  group('E2bWorkspaceService bootstrap (mock envd)', () {
    test(
      'ensureOpenCodeRunning maps exit code 42 to install failure',
      () async {
  ...
  group('E2bWorkspaceService isCloudUrl tests', () {
    test('identifies cloud and self-hosted URLs correctly', () {
      ...
      expect(
        E2bWorkspaceService.isCloudUrl('https://49983-sbx-abc.e2b.app'),
        isTrue,
      );
  ...
  group('E2bWorkspaceService connectSandbox tests', () {
    ...
    test(
      'connectSandbox happy path with existing sandbox and mock healthy server',
      () async {
        service.healthDioForTest = Dio()
          ..httpClientAdapter = FakeHttpAdapter((options, body) async {
            return ResponseBody.fromString('{"healthy":true}', 200);
    ...
    test(
      'connectSandbox recovers password from sandbox when missing in config',
      () async {
        service.healthDioForTest = Dio()
          ..httpClientAdapter = FakeHttpAdapter((options, body) async {
            return ResponseBody.fromString('{"healthy":true}', 200);
