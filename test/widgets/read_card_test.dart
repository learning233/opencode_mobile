...
Part _readPart({
  ...
  return Part(
    ...
    raw: {
      ...
      'state': {
        'status': status,
        'input': <String, dynamic>{'filePath': filePath, 'offset': 10, 'limit': 20},
...
void main() {
  ...
  testWidgets('ReadCard renders filename and opens file on tap without locking line', (tester) async {
    final part = _readPart(status: 'completed');

    await tester.pumpWidget(
      GetMaterialApp(
        home: Scaffold(
          body: ReadCard(part: part),
        ),
      ),
    );
